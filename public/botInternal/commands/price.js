...
function addPriceCommand(bot) {
    if (bot && typeof bot.get === 'function') {
        bot.get(/(\[club168593903.*\].*|^)(price[\s]|p[\s])/i, (message) => {
            let cardName = message.text.match(/(\[club168593903.*\].*|^)(price[\s]|p[\s])(.*)/i)[3];
            const setNameRegex = message.text.match(/(\[club168593903.*\].*|^)(price[\s]|p[\s])(.*)\[(.{3,4})\]/i);
