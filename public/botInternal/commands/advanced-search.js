...
function addAdvancedSearchCommand(bot) {
    if (bot && typeof bot.get === 'function') {
        bot.get(/(\[club168593903.*\].*|^)(advancedsearch[\s]|as[\s])/i, message => {
            //VK replaces quotes "" with &quot; characters, so we replace them back again
            const searchQuery = message.text.match(/(\[club168593903.*\].*|^)(advancedsearch[\s]|as[\s])(.*)/i)[3].replace(new RegExp('&quot;', 'g'), '"')
